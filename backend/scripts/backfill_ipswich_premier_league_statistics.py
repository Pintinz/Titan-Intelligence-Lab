"""Fixes a real gap found live-verifying the "teams without recent form" fix
(`backfill_efl_championship_history.py`): 6 of the 84 real upcoming fixtures involving
Coventry City FC / Hull City AFC / Sunderland AFC are `MISSING_REQUIRED_FEATURE`-blocked not
because the target team lacks history, but because their opponent in that fixture — Ipswich
Town FC — has zero `TeamStatistics` rows for any of its 2024-25 Premier League matches (verified
live: `form_shots_on_target_diff_last5` needs BOTH sides to have stats to compute a diff).
Ipswich's PL 2024-25 fixtures already exist in dev.db with real final scores (from an earlier
live-provider sync) — only match statistics were never ingested for them.

Source: football-data.co.uk's real, free, no-authentication-required Premier League 2024-25
results CSV — same source/shape as the EFL Championship backfill, filtered to Ipswich Town FC's
38 real matches only (home + away). Deliberately NOT importing the full 380-row season: the other
19 clubs' matches would need their own verified team-name rename map before touching them, and
they play no role in the gap this script fixes. `reconcile_fixture(match_by_teams_and_date=True,
preserve_existing_score=True)` matches these rows onto the fixtures that already exist and never
overwrites their real recorded score — this run only adds the missing statistics.

Team-name safety: verified live against dev.db (2026-08-20) — of the 19 real opponents in
Ipswich's 38 rows, only "Man City" / "Man United" / "Nott'm Forest" fail `_normalize_name`
matching against TitanIQ's existing "Manchester City" / "Manchester United" / "Nottingham
Forest" rows; every other name already matches exactly after normalization. "Ipswich" itself
needs renaming to TitanIQ's existing "Ipswich Town FC" row.

Read-only until `--import` is passed — defaults to DRY_RUN.
"""
...
def _premier_league_source() -> CsvHistoricalSource:
    return CsvHistoricalSource(
        source_key="football_data_co_uk_E0_2425_ipswich",
        ...
        default_competition_ref="premier_league",
...
async def main(csv_path: str, mode: ImportMode) -> None:
    ...
    report = await service.import_file(_premier_league_source(), rewritten_path, SportCode.FOOTBALL, mode, now)
