"""Tests the real OpenAI HTTP adapter against a mocked transport — the actual adapter code
(URL construction, request body, JSON parsing) runs; only the network is faked, mirroring
test_claude_adapter.py's/test_gemini_adapter.py's own established pattern (docs/decisions.md
ADR-008)."""
...
from modules.intelligence.infrastructure.openai_adapter import OpenAIAdapter, OpenAIRequestError
...
async def test_extract_events_parses_json_response():
    payload = {"events": [{"event_type": "injury", "summary": "Player injured.", "entities": ["p1"], "confidence": 0.8}]}

    def handler(request):
        return _openai_text_response(json.dumps(payload))

    adapter = OpenAIAdapter(get_api_key=_get_key, client=_client_for(handler))
...
async def test_extract_events_strips_a_markdown_fence_if_present():
    """OpenAI's response_format=json_object is server-enforced, so this should be rare — kept as
    the same honest safety net ClaudeAdapter's own equivalent test documents."""
    payload = {"events": [{"event_type": "transfer", "summary": "Signed.", "entities": [], "confidence": 0.6}]}

    def handler(request):
        return _openai_text_response(f"```json\n{json.dumps(payload)}\n```")
...
async def test_extract_events_raises_on_non_json_response():
    def handler(request):
        return _openai_text_response("not json")
...
async def test_summarize_returns_plain_text():
    def handler(request):
        return _openai_text_response("A short summary.")
...
async def test_explain_returns_plain_text():
    def handler(request):
        return _openai_text_response("This prediction is driven by recent form.")
...
async def test_interpret_sentiment_normalizes_casing():
    def handler(request):
        return _openai_text_response("  POSITIVE  ")
...
async def test_extract_entities_parses_json_response():
    payload = {"entities": [{"text": "Erling Haaland", "entity_type": "player", "confidence": 0.9}]}
...
async def test_extract_relationships_parses_json_response():
    payload = {"relationships": [{"subject": "Mbappe", "relation": "transferred_to", "object": "Real Madrid", "confidence": 0.7}]}
...
@pytest.mark.asyncio
async def test_classify_topics_parses_json_object_with_topics_key():
    def handler(request):
        return _openai_text_response(json.dumps({"topics": ["injury", "transfer"]}))
...
async def test_detect_language_normalizes_to_two_letter_code():
    def handler(request):
        return _openai_text_response("EN")
...
async def test_extract_key_phrases_parses_json_and_respects_limit():
    payload = {"phrases": [{"phrase": "injury", "score": 0.9}, {"phrase": "transfer", "score": 0.7}, {"phrase": "tactics", "score": 0.5}]}
...
async def test_default_model_used_when_not_specified():
    ...
    def handler(request):
        ...
        return _openai_text_response("ok")
...
@pytest.mark.asyncio
async def test_json_response_requests_are_sent_with_json_object_response_format():
    captured = {}

    def handler(request):
        captured["body"] = json.loads(request.content)
        return _openai_text_response(json.dumps({"topics": []}))
...
# -- 2026-08-30: openai_call_count metric ---------------------------------------------------------
...
async def test_real_call_increments_the_metrics_recorder():
    ...
    adapter = OpenAIAdapter(get_api_key=_get_key, client=_client_for(handler), metrics_recorder=recorder)

    await adapter.summarize("Some text.")

    assert recorder.openai_call_count == 1
    assert recorder.gemini_call_count == 0  # never conflated with another provider's counter
    assert recorder.claude_call_count == 0
...
@pytest.mark.asyncio
async def test_assess_prediction_context_returns_raw_openai_text():
    raw = '{"prediction_review": {"status": "SUPPORTED"}}'

    def handler(request):
        return _openai_text_response(raw)
...
async def test_assess_prediction_context_uses_the_shared_prompt_as_a_system_message():
    """Same instructions as GeminiAdapter's/ClaudeAdapter's own — sent as a system message, not
    concatenated into the user message, but textually identical (llm_prompts.py is the one shared
    source, imported by all three adapters) so no provider can ever narrate under different rules
    for the same job."""
    captured = {}
    ...
    system_message = next(m["content"] for m in captured["body"]["messages"] if m["role"] == "system")
    user_message = next(m["content"] for m in captured["body"]["messages"] if m["role"] == "user")
    assert "NOT the primary statistical prediction model" in system_message
    assert "football.match_winner" in user_message
...
async def test_explain_football_prediction_uses_the_football_analyst_prompt():
    ...
    system_message = next(m["content"] for m in captured["body"]["messages"] if m["role"] == "system")
    assert "TitanIQ's football sports analyst" in system_message
...
async def test_no_recorder_configured_does_not_raise():
    ...
    adapter = OpenAIAdapter(get_api_key=_get_key, client=_client_for(handler))  # no metrics_recorder
