...
from modules.predictions.basketball.market_seeding import MARKETS, SINGLE_RECORD_FEATURES, BasketballMarketSeeder
...
def seeder(registration, store, market_registry, mapping_service, team_statistics_repo):
    return BasketballMarketSeeder(
        ...
        windowed_calculator=basketball_form_calculator(registration, store, team_statistics_repo),
        differential_calculator=basketball_fixture_form_differential_calculator(registration, store, team_statistics_repo),
    )
...
async def test_seed_registers_windowed_feature(seeder, feature_definition_repo):
    ...
    definition = await feature_definition_repo.get(FeatureKey("basketball.team.form_points_last5"))
    assert definition is not None
    assert definition.is_consumable()
...
@pytest.mark.asyncio
async def test_seed_covers_seven_distinct_market_kinds(seeder, market_repo):
    await seeder.seed(T0)

    kinds = {spec["market_kind"] for spec in MARKETS}
    assert len(kinds) == 7
...
async def test_seed_is_idempotent(seeder, market_repo):
    ...
    markets = await market_repo.list_by_sport("basketball")
