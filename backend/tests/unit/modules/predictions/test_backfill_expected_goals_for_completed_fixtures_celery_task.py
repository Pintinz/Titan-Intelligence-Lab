...
@dataclass
class _FakeExpectedGoalsCalculator:
    ...
    async def compute_and_write(self, fixture_id, home_team_id, away_team_id, cutoff):
...
def _completed_fixture_row(scheduled_at=None, tz_aware=True):
    ...
    return (str(uuid4()), str(uuid4()), str(uuid4()), kickoff)
...
@pytest.fixture
def fake_calculator():
    return _FakeExpectedGoalsCalculator()
...
def fake_context(fake_calculator, fake_session):
    return tasks_module.MarketFeatureRepairContext(
        seeder=None, mappings=None, venue_strength_calculator=None,
        expected_goals_calculator=fake_calculator, session=fake_session,
    )
...
def test_backfills_every_completed_fixture_using_its_own_kickoff_as_cutoff(
    ...
    result = tasks_module.backfill_expected_goals_for_completed_fixtures_task.apply(
    ...
    # Point-in-time safety: each fixture's own scheduled_at is the cutoff, never `now` — the same
    # leakage class every sibling backfill task in this file guards against.
    for (_, cutoff), (_, _, _, scheduled_at) in zip(fake_calculator.compute_calls, completed_rows):
    ...
    assert result["expected_goals_backfilled"] == len(completed_rows)
...
def test_excludes_already_covered_fixtures_by_default_but_not_when_forced(fake_calculator, fake_session):
    tasks_module.backfill_expected_goals_for_completed_fixtures_task.apply(kwargs={"now_iso": T0.isoformat()}).get()
    assert "NOT EXISTS" in fake_session.last_discovery_query
    assert "expected_home_goals" in fake_session.last_discovery_query

    tasks_module.backfill_expected_goals_for_completed_fixtures_task.apply(
...
def test_commits_in_batches_instead_of_only_once_at_the_end(fake_calculator):
    batch_size = tasks_module._EXPECTED_GOALS_BACKFILL_COMMIT_BATCH_SIZE
    ...
    async def factory():
        return tasks_module.MarketFeatureRepairContext(
            seeder=None, mappings=None, venue_strength_calculator=None,
            expected_goals_calculator=fake_calculator, session=session,
        )
...
def test_a_fixture_the_calculator_cant_compute_for_is_not_counted_as_backfilled(fake_calculator, completed_rows):
    ...
    assert result["expected_goals_backfilled"] == len(completed_rows) - 1
    assert result["completed_football_fixtures_checked"] == len(completed_rows)
...
def test_naive_scheduled_at_is_stamped_utc_not_left_ambiguous(fake_calculator):
    ...
    async def factory():
        return tasks_module.MarketFeatureRepairContext(
            seeder=None, mappings=None, venue_strength_calculator=None,
            expected_goals_calculator=fake_calculator, session=_FakeSession(rows=[naive_row]),
        )
    ...
    assert cutoff == naive_row[3].replace(tzinfo=timezone.utc)


def test_task_defaults_now_to_current_time_when_not_supplied(fake_calculator):
    tasks_module.backfill_expected_goals_for_completed_fixtures_task.apply(kwargs={}).get()
    assert len(fake_calculator.ensure_registered_calls) == 1
...
def test_task_times_out_instead_of_hanging_forever(monkeypatch, fake_calculator):
    monkeypatch.setattr(tasks_module, "_EXPECTED_GOALS_BACKFILL_TASK_TIMEOUT_SECONDS", 0.05)

    async def factory():
        return tasks_module.MarketFeatureRepairContext(
            seeder=None, mappings=None, venue_strength_calculator=None,
            expected_goals_calculator=fake_calculator, session=_HangingSession(),
        )
