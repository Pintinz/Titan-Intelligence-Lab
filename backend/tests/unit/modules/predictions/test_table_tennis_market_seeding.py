...
from modules.predictions.application.windowed_feature_engineering_service import table_tennis_form_calculator
from modules.predictions.domain.value_objects import MarketStatus
from modules.predictions.table_tennis.market_seeding import MARKETS, SINGLE_RECORD_FEATURES, TableTennisMarketSeeder
...
def seeder(registration, store, market_registry, mapping_service, team_statistics_repo):
    return TableTennisMarketSeeder(
        ...
        windowed_calculator=table_tennis_form_calculator(registration, store, team_statistics_repo),
    )
...
async def test_seed_registers_windowed_feature(seeder, feature_definition_repo):
    ...
    definition = await feature_definition_repo.get(FeatureKey("table_tennis.team.form_points_won_last5"))
    assert definition is not None
    assert definition.is_consumable()
...
@pytest.mark.asyncio
async def test_seed_covers_six_distinct_market_kinds(seeder):
    await seeder.seed(T0)
...
async def test_seed_is_idempotent(seeder, market_repo):
    ...
    markets = await market_repo.list_by_sport("table_tennis")
