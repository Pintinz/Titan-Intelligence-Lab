...
@lru_cache
def get_calibrator() -> CalibratorPort:
    """Process-wide singleton — `PlattScalingCalibrator` accumulates fitted per-model parameters
    in memory across requests; a fresh instance per request would forget every calibration
    `ModelRegistryService.promote_to_champion`-adjacent tooling has already fitted."""
