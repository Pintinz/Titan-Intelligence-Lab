...
class PredictionRepositoryPort(Protocol):
    ...
    async def get_many(self, prediction_ids: list[PredictionId]) -> list[Prediction]:
        """Batched `get()` — one query for many ids, not one query per id. Real production
        incident (2026-08-30): `CalibrationFittingService._fit_market` called `get()` once per
        outcome (up to 2000 per market, across every PRODUCTION market) inside a loop, an N+1
        query pattern that alone accounted for `predictions.check_scheduled_calibration` blowing
        past its 300s task timeout. Order is not guaranteed to match `prediction_ids`; a missing
        id is simply absent from the result, never an error (same "honest gap, not a crash"
        posture `get()` itself already has for a single missing id)."""
        ...
