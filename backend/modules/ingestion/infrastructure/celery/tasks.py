...
def sync_countries_task(self, sport_code: str, now_iso: str) -> dict | None:
    async def _do():
        orchestrator = await _get_orchestrator()
        return await orchestrator.sync_countries(sport_code, datetime.fromisoformat(now_iso))

    return _run_summary(asyncio.run(_do()))


@celery_app.task(name="ingestion.sync_teams", bind=True, queue="default", **_RETRY_KWARGS)
def sync_teams_task(self, sport_code: str, competition_ref: str, now_iso: str) -> dict | None:
    async def _do():
        orchestrator = await _get_orchestrator()
        return await orchestrator.sync_teams(sport_code, competition_ref, datetime.fromisoformat(now_iso))

    return _run_summary(asyncio.run(_do()))


@celery_app.task(name="ingestion.sync_fixtures", bind=True, queue="default", **_RETRY_KWARGS)
def sync_fixtures_task(self, sport_code: str, competition_ref: str, season_label: str, season_id_str: str, now_iso: str) -> dict | None:
    async def _do():
        orchestrator = await _get_orchestrator()
...
def sync_live_fixtures_task(self, sport_code: str, competition_ref: str, season_label: str, season_id_str: str, now_iso: str) -> dict | None:
    async def _do():
        orchestrator = await _get_orchestrator()
