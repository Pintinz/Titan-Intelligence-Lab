...
/**
 * CompetitionHero — the Competition Center's opening instrument. Stadium-lighting glow blobs
 * (reusing the `animate-hero-glow` keyframe already proven on Mission Control/Team Hero) plus a
 * faint telemetry grid stand in for "knowledge graph particles" — real atmosphere, not a stock
 * photo. Real competition crests already on screen elsewhere on this page also drift faintly in
 * the backdrop instead of stock league artwork, so nothing here is invented imagery.
 * Search + sport switcher live together as one instrument, satisfying the brief's "quick filters"
 * and "premium segmented control" as a single control rather than two competing pickers.
 */
export function CompetitionHero({
  ...
  sport: SportMeta
  /** Omitted on sport-scoped routes (e.g. `/app/football/competitions`) — SportShell already owns
   * sport switching there, so the segmented control only renders when a handler is supplied. */
      ...
      <div className="relative flex flex-col gap-6">
        <div>
          ...
          <p className="mt-2.5 max-w-xl font-[var(--cd-font-body)] text-[13.5px] leading-relaxed sm:text-[15px]" style={{ color: 'var(--cd-text-secondary)' }}>
            Explore every league, tournament and championship covered by TitanIQ's AI Intelligence Engine.
          </p>
        </div>
