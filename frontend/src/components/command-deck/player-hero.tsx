 ...
 * atmosphere as Team/Competition Hero. Sport-scoped only (there is no cross-sport `/app/players`
 * destination), so unlike `TeamHero`/`CompetitionHero` this never renders a sport switcher —
 * `SportShell` already owns that one level up. Backdrop crests are the same sport's real team
 * logos (joined in by `usePlayerIntelligence`), never stock player photography — the backend
 * exposes no player headshots.
 */
